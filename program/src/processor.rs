...
//fixme done
//simply packs lending market state info into a lending_market_info account
...
//fixme done
#[inline(never)] // avoid stack frame limit todo learn more
...
// in total involves the following:
// 1) get market price for collateral via pyth
// 2) write Reserve state
// 3) move collateral (eg ETH) from user to lending protocol
// 4) mint LP tokens back to user
fn process_init_reserve(
    ...
    // todo temp disabling for testing
    ...
    //user's accounts
    let source_liquidity_info = next_account_info(account_info_iter)?; //User's account with collateral tokens
    let destination_collateral_info = next_account_info(account_info_iter)?; //User's account for LP tokens
    //collateral tokens
    let reserve_info = next_account_info(account_info_iter)?; //Reserve state holder
    let reserve_liquidity_mint_info = next_account_info(account_info_iter)?; //Mint info for collateral tokens
    let reserve_liquidity_supply_info = next_account_info(account_info_iter)?; //Account holding collateral tokens
    let reserve_liquidity_fee_receiver_info = next_account_info(account_info_iter)?; //Account getting fees in collateral tokens
    //todo found a bug here - pyth accounts should come before reserve collateral
    //pyth
    let pyth_product_info = next_account_info(account_info_iter)?;
    let pyth_price_info = next_account_info(account_info_iter)?; //oracle account for tokens used as collateral
    //LP tokens
    let reserve_collateral_mint_info = next_account_info(account_info_iter)?; //Mint info for LP tokens
    let reserve_collateral_supply_info = next_account_info(account_info_iter)?; //Account holding LP tokens
    //lending market
    let lending_market_info = next_account_info(account_info_iter)?; //LendingMarket state holder
    let lending_market_authority_info = next_account_info(account_info_iter)?; //11
    let lending_market_owner_info = next_account_info(account_info_iter)?;
    ...
    //get the init'ed lending market
    ...
    if &lending_market.oracle_program_id != pyth_product_info.owner {
        ...
        msg!("{},{}", &lending_market.oracle_program_id, pyth_product_info.owner);
    ...
    if &lending_market.oracle_program_id != pyth_price_info.owner {
        ...
        msg!("{},{}", &lending_market.oracle_program_id, pyth_price_info.owner);
        return Err(LendingError::InvalidOracleConfig.into());
    }

    // --------------------------------------- pyth

    //load pyth as a whole
    ...
    //ultimately we arrive at market price for tokens used as collateral
    let market_price = get_pyth_price(pyth_price_info, clock)?;

    // --------------------------------------- reserve authority

    //when we CREATE the PDA, we pass in BOTH our seed AND the bump seed
    let authority_signer_seeds = &[
        lending_market_info.key.as_ref(),
        &[lending_market.bump_seed],
    ];
    let lending_market_authority_pubkey = Pubkey::create_program_address(
        authority_signer_seeds, program_id)?;
    if &lending_market_authority_pubkey != lending_market_authority_info.key {
        msg!("seeds are {:?}", authority_signer_seeds);
        msg!("{} <----> {}", lending_market_authority_pubkey, lending_market_authority_info.key);
        msg!(
    ...
    reserve.init(InitReserveParams {
        ...
        //available amount = 0, borrowed amount = 0, liquidity rate = 1
        liquidity: ReserveLiquidity::new(NewReserveLiquidityParams {
            ...
            market_price, //which we just retrieved via Pyth. So we include the market price of the underlying when we init the reserve
        }),
        //total supply = 0
        ...
        config, //passed into the function
    });

    //immediately add liquidity to the reserve - increments available_amount and total_collateral_amount
    let collateral_amount = reserve.deposit_liquidity(liquidity_amount)?;
    Reserve::pack(reserve, &mut reserve_info.data.borrow_mut())?;

    //init the reserve liquidity SUPPLY account
    ...
    //init the reserve liquidity FEE account
    spl_token_init_account(TokenInitializeAccountParams {
    ...
    //init the destination LP account - I guess this is where LP tokens for initial liquidity will go
    spl_token_init_account(TokenInitializeAccountParams {
    ...
    //move tokens from source liquidity account to reserve liquidity SUPPLY account
    ...
    //mint LP tokens to the destination account
    ...
    //thus:
    //move eg ETH source_liquidity_info -> reserve_liquidity_supply_info
    //mint new LP tokens -> destination_collateral_info
...
fn process_refresh_reserve(program_id: &Pubkey, accounts: &[AccountInfo]) -> ProgramResult {
    ...
    //finally update slot with the current slot
...
fn process_deposit_reserve_liquidity(
    ...
    //unpack state of lending market
    let lending_market = LendingMarket::unpack(&lending_market_info.data.borrow())?;
    if lending_market_info.owner != program_id {
        msg!("Lending market provided is not owned by the lending program");
        return Err(LendingError::InvalidAccountOwner.into());
    ...
    // record deposited liquidity and return the number of collateral tokens to mint
    let collateral_amount = reserve.deposit_liquidity(liquidity_amount)?;

    // --------------------------------------- execute

    // mark the reserve as stale and pack it back in
    reserve.last_update.mark_stale();
    Reserve::pack(reserve, &mut reserve_info.data.borrow_mut())?;

    // move tokens from user's account to lending protocol's account
    spl_token_transfer(TokenTransferParams {
        source: source_liquidity_info.clone(),
        destination: reserve_liquidity_supply_info.clone(),
        amount: liquidity_amount,
...
fn process_redeem_reserve_collateral(
    ...
    //move the collateral tokens back to the user
    spl_token_transfer(TokenTransferParams {
...
fn process_init_obligation(program_id: &Pubkey, accounts: &[AccountInfo]) -> ProgramResult {
    let account_info_iter = &mut accounts.iter();
    //holds the state of the obligation
    ...
    //initiate it - ie write the state to it
    obligation.init(InitObligationParams {
        current_slot: clock.slot,
        lending_market: *lending_market_info.key,
        owner: *obligation_owner_info.key,
        deposits: vec![], //empty deposits
        borrows: vec![], //empty borrows
    });
...
fn process_refresh_obligation(program_id: &Pubkey, accounts: &[AccountInfo]) -> ProgramResult {
    let account_info_iter = &mut accounts.iter().peekable();
    let obligation_info = next_account_info(account_info_iter)?; //state of obligation
    let clock = &Clock::from_account_info(next_account_info(account_info_iter)?)?;
    ...
    // we're going to be re-calculating these from scratch
    ...
    //next we iterate through all deposits and all borrows and we add the values to the values above
    for (index, collateral) in obligation.deposits.iter_mut().enumerate() {
        ...
        //this is the main thing. we refresh the market value -> which dictates borrow amount and liquidation amount
        let market_value = deposit_reserve
            .collateral_exchange_rate()? //LP token supply / liquidity available
            .decimal_collateral_to_liquidity(collateral.deposited_amount.into())? //LP amount divide by above = liquidity amount
            //liq amnt x price x decimals
            .try_mul(deposit_reserve.liquidity.market_price)?
            .try_div(decimals)?;

        collateral.market_value = market_value;

        //just pull stuff from config
        ...
        //calc deposited value, allowed borrow value, liquidation value
        deposited_value = deposited_value.try_add(market_value)?;

        // each reserve sets a loan:value ratio, which decides how much you can borrow given how much collateral you've deposited
        allowed_borrow_value =
            allowed_borrow_value.try_add(market_value.try_mul(loan_to_value_rate)?)?;

        // each reserve also sets a threshold at which LTV is considered unhealthy, and at which the obligation can be liquidated
...
fn process_deposit_obligation_collateral(
    ...
    //mark oblig stale & write its state
    obligation.last_update.mark_stale();
    Obligation::pack(obligation, &mut obligation_info.data.borrow_mut())?;

    //finally move tokens from user's collateral account to protocol's
    spl_token_transfer(TokenTransferParams {
        source: source_collateral_info.clone(),
        destination: destination_collateral_info.clone(),
        amount: collateral_amount,
...
fn process_withdraw_obligation_collateral(
    ...
    //since we're withdrawing this means the deposit acc definitely exists
    ...
    let withdraw_amount = if obligation.borrows.is_empty() {
        ...
        //if they never deposited anything = error
        ...
        //the interesting case - when they do have borrows outstanding
    } else {
        //value = measured in $
        //amount = measured in coins

        //first we get value (in $)
        ...
        //then we convert it to amount (in coins)
        //all the dancing around with u64 and the circularity is to make sure we don't somehow send the user more than they should get
        //this makes sense - ti's a tricky place in the code, if this fails, the protocol fails
        let withdraw_amount = if collateral_amount == u64::MAX {
            //the smallest of max allowed withdraw value and collateral's market value
            let withdraw_value = max_withdraw_value.min(collateral.market_value);
            // withdraw value / collateral total value * collateral deposited amount
    ...
    //actually transfer the collateral tokens back to the user
    spl_token_transfer(TokenTransferParams {
        source: source_collateral_info.clone(),
        destination: destination_collateral_info.clone(),
...
fn process_borrow_obligation_liquidity(
    ...
    let CalculateBorrowResult {
        borrow_amount, //below+fees
        receive_amount, //requested by user
        borrow_fee,
        host_fee, //host fee is inside the borrow fee
    ...
    //update reserve
    //subtracts from available amount and increments borrowed amount
    ...
    if let Ok(host_fee_receiver_info) = next_account_info(account_info_iter) {
        if host_fee > 0 {
            //owner fee = owner fee - host fee
            owner_fee = owner_fee
                .checked_sub(host_fee)
                .ok_or(LendingError::MathOverflow)?;
            // actually send fee to host
            spl_token_transfer(TokenTransferParams {
                source: source_liquidity_info.clone(),
    ...
    //finally send the borrowed funds to the user
    spl_token_transfer(TokenTransferParams {
        source: source_liquidity_info.clone(),
        destination: destination_liquidity_info.clone(),
...
fn process_repay_obligation_liquidity(
    ...
    //find the relevant liquidity basket
    ...
    //the two numbers are basically the same, just one is a decimal and one a u64
    ...
    //increment availalble pool, reduce outstanding pool
    repay_reserve.liquidity.repay(repay_amount, settle_amount)?; //it's kinda stupid that we have to pass both even though both are the same value
    repay_reserve.last_update.mark_stale();
    Reserve::pack(repay_reserve, &mut repay_reserve_info.data.borrow_mut())?;

    //reduce borrowed amounts in user's obligation
...
fn process_liquidate_obligation(
    ...
    //liquidation bot
    let destination_collateral_info = next_account_info(account_info_iter)?;
    //reserve for supply tokens state
    let repay_reserve_info = next_account_info(account_info_iter)?;
    //reserve for supply token acc
    let repay_reserve_liquidity_supply_info = next_account_info(account_info_iter)?;
    //reserve for collateral tokens state
    let withdraw_reserve_info = next_account_info(account_info_iter)?;
    //reserve for supply tokens acc
    ...
    //find the relevant collateral basket
    let (collateral, collateral_index) =
        obligation.find_collateral_in_deposits(*withdraw_reserve_info.key)?;
    ...
    let CalculateLiquidationResult {
        settle_amount, //in liquidity terms
        repay_amount, //in liquidity terms
        withdraw_amount, // in collateral terms
    ...
    //https://academy.ivanontech.com/blog/defi-deep-dive-decentralized-money-markets-and-compound#:~:text=In%20Compound%2C%20liquidation%20occurs%20if,in%20and%20liquidate%20your%20position.
    //They do this to eliminate any risk to the protocol by repaying up to 50% of the assets you borrowed and they will also help themselves to a portion of your collateral at the market price minus the liquidation discount.
    //so yes, the bot is sending in liquidity, while taking the collateral for himself

    //liquidator bot sends liquidity back to the protocol
    spl_token_transfer(TokenTransferParams {
        source: source_liquidity_info.clone(), //this is the bot
    ...
    //protocol sends borrower's collateral to liquidation bot
    spl_token_transfer(TokenTransferParams {
        source: withdraw_reserve_collateral_supply_info.clone(),
        destination: destination_collateral_info.clone(), //this is the bot
        amount: withdraw_amount,
        authority: lending_market_authority_info.clone(),
        authority_signer_seeds,
        token_program: token_program_id.clone(),
...
fn process_flash_loan(
    ...
    //expect the liquidity provider to have a fee in their account after this is done
    ...
    //will have to return with a fee
    let returned_amount_required = flash_loan_amount
        .checked_add(origination_fee)
        .ok_or(LendingError::MathOverflow)?;

    // --------------------------------------- flash loan specific exec

    //prep all the accounts required
    ...
    //mark liquidity as unavilable
    reserve.liquidity.borrow(flash_loan_amount_decimal)?;
    Reserve::pack(reserve, &mut reserve_info.data.borrow_mut())?;

    //transfer liquidity from protocol to user
    spl_token_transfer(TokenTransferParams {
        source: source_liquidity_info.clone(),
        destination: destination_liquidity_info.clone(),
    ...
    //prepare data for a custom instruction related to flash loans
    ...
    //invoke a custom flash loan program with accounts and data
    invoke(
        &Instruction {
    ...
    //make sure that the protocol's liquidity acc has the old amount + fee in there for doing the flash loan
    ...
    //pay out the fees, similarly to how they're paid when a normal borrow happens (yes fees are paid when borrowing, not repaying)
...
// fixme done
// checks if rent above min balance to be rent exempt
...
// fixme done
// simply checks if is_intiialized is set to true on a struct
...
fn get_pyth_price(pyth_price_info: &AccountInfo, clock: &Clock) -> Result<Decimal, ProgramError> {
    //seems like we give our api call an expiration
    ...
    //actual price (decimal) = price (u64) * 10**exponent
    let market_price = if pyth_price.expo >= 0 {
        //if expo > 0, then price > 1
...
fn spl_token_init_account(params: TokenInitializeAccountParams<'_>) -> ProgramResult {
    let TokenInitializeAccountParams { //destructure, this is cool, didn't know you could do it this way
        account,
        mint,
        owner,
        rent,
