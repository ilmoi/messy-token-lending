...
impl Reserve {
    ...
    pub fn deposit_liquidity(&mut self, liquidity_amount: u64) -> Result<u64, ProgramError> {
        let collateral_amount = self
            .collateral_exchange_rate()? //total liquidity / total collateral supply
            .liquidity_to_collateral(liquidity_amount)?; //above * liquidity amount
    ...
    pub fn redeem_collateral(&mut self, collateral_amount: u64) -> Result<u64, ProgramError> {
        let collateral_exchange_rate = self.collateral_exchange_rate()?; //LP supply / liquidity available

        // liquidity available to withdraw = LP tokens * liquidity available / LP supply
    ...
    //todo interesting - the higher the utilization, the higher the borrow rate
    // not a crazy idea - the more ppl want to borrow something, the higher the rate
    /// Calculate the current borrow rate
    pub fn current_borrow_rate(&self) -> Result<Rate, ProgramError> {
        let utilization_rate = self.liquidity.utilization_rate()?; //calc'ed as borrowed coins / total coins
        ...
        //if low utilization, then by definition rate should be higher to attract users
        if low_utilization || self.config.optimal_utilization_rate == 100 {

            /// eg util rate = 20%
            /// optimal util rate = 50%
            /// normalized rate = 20% / 50% = 40%
            /// min rate = 10%
            /// rate range = 50-10=40%
            /// 40% * 40% + 10% = 26%
            ///
            /// eg util rate = 40%
            /// optimal util rate = 50%
            /// normalized rate = 40% / 50% = 80%
            /// min rate = 10%
            /// rate range = 50-10=40%
            /// 80% * 40% + 10% = 42%

            // this is basically what % of ideal rate is actually used
            ...
            //optimal less minimal borrow rate
            ...
            // normalized rate * rate range + min rate
            ...
            //(util rate - optimal util rate) / (100-optimal util rate)
    ...
    //fixme done - LP token supply / liquidity available
    ...
    //todo kinda done - still not sure what the params are
    ...
    pub fn accrue_interest(&mut self, current_slot: Slot) -> ProgramResult {
        ...
        if slots_elapsed > 0 {
            ...
            //updates 2 params:
            //1) cumulative_borrow_rate_wads
            //2) borrowed_amount_wads
            self.liquidity
    ...
    pub fn calculate_borrow(
        ...
        if amount_to_borrow == u64::MAX {
            ...
            //calc fees and add them to the borrow amount
            let (borrow_fee, host_fee) = self
                .config
                .fees
                .calculate_borrow_fees(borrow_amount, FeeCalculation::Exclusive)?;

            //(!) note we're only adding the borrow fee. this means that hsot fee is PART OF THE BORROW FEE
            let borrow_amount = borrow_amount.try_add(borrow_fee.into())?;

            //make sure user isn't borrowing too much money (in $)
            ...
            Ok(CalculateBorrowResult {
                borrow_amount, //this is the below + fees
                receive_amount, //this is the amount the user requested and will receive
                borrow_fee,
                host_fee,
            })
        }
    ...
    //fixme done - in short takes the min of borrowed amount and amount to repay
    ...
    pub fn calculate_repay(
        ...
        //ok I think this U64::MAX logic is needed for the "max" button on the website to work
        let settle_amount = if amount_to_repay == u64::MAX {
            //if amount to repay is already the max u64 digit, then we
            borrowed_amount
        } else {
            //min of amount_to_repay or borrowed_amount
            //we have to convert to decimal to do the comparison
            Decimal::from(amount_to_repay).min(borrowed_amount)
        };
        //then we convert back to u64, but using ceiling not floor, ie the user might pay back a little more
        let repay_amount = settle_amount.try_ceil_u64()?;

        //the two numbers are the same, just diff types
    ...
    pub fn calculate_liquidation(
        ...
        //so our liquidators get a % bonus for successfully liquidating a deliquent loan
        //bonus rate = 1 + rate
        ...
        let max_amount = if amount_to_liquidate == u64::MAX {
            ...
            //the min of how much we want to liquidate and the borrowed amount
            Decimal::from(amount_to_liquidate).min(liquidity.borrowed_amount_wads)
        };

        let settle_amount; //goes to lender, in terms of liquidity, decimal
        let repay_amount; //goes to lender, in terms of liquidity, u64
        let withdraw_amount; //goes to lender, in terms of collateral, u64

        // Close out obligations that are too small to liquidate normally
        // the difference between if and else here is that
        //  - the if case liquidates the entire position because it's too small
        //  - the else case liquidates only 50% of the position, because that's the liquidation factor. //todo confirm this is how liquidation factors work
        if liquidity.borrowed_amount_wads < LIQUIDATION_CLOSE_AMOUNT.into() {
            ...
            let liquidation_amount = obligation
                .max_liquidation_amount(liquidity)? //50% of original position value
                .min(max_amount);
            ...
            let liquidation_value = liquidity
                ...
                .try_mul(bonus_rate)?; // (!) note we're not only multiplying by liquidation percentage but also adding the bonus rate to the liquidator
                // paid out to the bot because included in the withdrawal amount
                // todo weird, but aren't they also sending in more themselves? isn't this value used to calc settle amount / repay amount which the bot sends IN?

            match liquidation_value.cmp(&collateral.market_value) {
                //insufficient collateral
                Ordering::Greater => {
                    let repay_pct = collateral.market_value.try_div(liquidation_value)?; //this number always <1
                ...
                Ordering::Less => {
                    let withdraw_pct = liquidation_value.try_div(collateral.market_value)?; //this number always <1
                    settle_amount = liquidation_amount;
        ...
        Ok(CalculateLiquidationResult {
            settle_amount, //owed to lender, decimal
            repay_amount, //owed to lender, u64
            withdraw_amount, //actually goes to lender
        })
    }
}
...
pub struct ReserveLiquidity {
    ...
    pub available_amount: u64, //todo wonder why store this as a u64 not as a decimal?
    /// Reserve liquidity borrowed
    pub borrowed_amount_wads: Decimal,
    /// Reserve liquidity cumulative borrow rate
    /// todo not sure I understand what this actually means
...
impl ReserveLiquidity {
    ...
    //fixme done - when 1 user borrows from the reserve that liquidity is no longer availalbe to other users for borrowing
    ...
    pub fn borrow(&mut self, borrow_decimal: Decimal) -> ProgramResult {
        ...
        //convert decimal to u64 - we need to do it to be able to compare to self.available_amount, which is a u64
        ...
        //it feels weird that we're adding decimal here, but it's actually the right way to do it
        //decimal = huge U192 number that has 18 digits after comma.
        //it's like a super precise number - which is why we're adding it to other super precise numbers
        self.borrowed_amount_wads = self.borrowed_amount_wads.try_add(borrow_decimal)?;

        Ok(())
    }

    //fixme done - reduce borrowed amount, add back to available amount
    ...
    //fixme done - basically borrowed coins / total available coins
    /// Calculate the liquidity utilization rate of the reserve
    pub fn utilization_rate(&self) -> Result<Rate, ProgramError> {
        let total_supply = self.total_supply()?; //reserve liquidity available
        if total_supply == Decimal::zero() {
            return Ok(Rate::zero());
        }
        //reserve liquidity borrowed / reserve liquidity available
    ...
    fn compound_interest(
        ...
        //interest per slot = borrow rate / slots per year
        let slot_interest_rate = current_borrow_rate.try_div(SLOTS_PER_YEAR)?;
        //compound_interest = (1+interest)**slots_elapsed
        let compounded_interest_rate = Rate::one()
            .try_add(slot_interest_rate)?
            .try_pow(slots_elapsed)?;

        //cumulative_borrow_rate_wads = wads x compound_interest_rate
        //so eg 1.1 * 1.25 = 1.375
        self.cumulative_borrow_rate_wads = self
            .cumulative_borrow_rate_wads
            .try_mul(compounded_interest_rate)?;

        //borrowed_amount_wads = self x compound_interest_rate
        //this makes sense - as time passes borrowed "amount" "increases", in a sense that it's the original amount + added interest
        //so eg 1000 x 1.25 = 1250. You borrowed 1000, but now have to pay back 1250 because of interest
...
impl ReserveCollateral {
    ...
    //fixme done - returns mint total supply / total liquidity
    ...
    fn exchange_rate(
        ...
        let rate = if self.mint_total_supply == 0 || total_liquidity == Decimal::zero() {
            // currently set to 1 * WAD
            Rate::from_scaled_val(INITIAL_COLLATERAL_RATE)
        } else {
            //mint total supply / liquidity
...
//fixme done
//todo would be awesome to provide guidance on the below numbers in the readme
/// Reserve configuration values
...
impl ReserveFees {
    ...
    pub fn calculate_borrow_fees(
        ...
        //returns both protocol and host fees
        self.calculate_fees(borrow_amount, self.borrow_fee_wad, fee_calculation)
    }

    //todo like above just uses a different % fee
    ...
    fn calculate_fees(
        ...
        if borrow_fee_rate > Rate::zero() && amount > Decimal::zero() {
            ...
            //take the largest of the two - the calc'ed borrow fee or the minimum fee
            let borrow_fee = borrow_fee_amount.try_round_u64()?.max(minimum_fee);

            //make sure fees don't exceed total borrow amount
            ...
            //calc host fee, using host fee rate
