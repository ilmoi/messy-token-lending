...
impl Obligation {
    ...
    //todo remove from deposits - either the deposit entry entirely (if 0 left), or simply sub
    ...
    /// borrowed_value x deposited_value / allowed_borrowed_value (the marks works out, I checked)
    ...
    //todo returns amount to be repaid by borrower in wads
    ...
    pub fn max_liquidation_amount(
        ...
        let max_liquidation_value = self
            .borrowed_value
            .try_mul(Rate::from_percent(LIQUIDATION_CLOSE_FACTOR))? //so 50% of borrowed value needs to be repaid
            .min(liquidity.market_value); //smaller of that or market value
    ...
    pub fn find_or_add_collateral_to_deposits(
        ...
        //if collateral type already present in Deposits array, then return a mutable reference to it
        ...
        //otherwise create a fresh instance of oblig collateral and return it
        let collateral = ObligationCollateral::new(deposit_reserve);
        self.deposits.push(collateral);
        Ok(self.deposits.last_mut().unwrap())
    }

    //fixme done - tries to find the collateral with the reserve that matches, if so returns the index of that collateral
...
impl ObligationLiquidity {
    ...
    //todo increments borrowed amount
