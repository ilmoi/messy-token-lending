...
export class Blockchain {
    ...
    flashLoanProgram = new PublicKey("Eiy9gzpAcjQiav3q4QQNLFxRqCVFXiPboVwLSDS19UFc");
    ...
    async _prepareAndSendTx(instructions: TransactionInstruction[], signers: Signer[]) {
        ...
        let sig = await sendAndConfirmTransaction(this.connection, tx, signers);
        console.log(sig);
    }
    ...
    async _createAndFundUserAccount(mint: Token, mintAmount: number): Promise<PublicKey> {
        let tokenUserPk = await mint.createAccount(this.ownerKp.publicKey);
